...
parser.add_argument('--lr_steps', default=[24, 35, 45], type=int, nargs="+",
...
parser.add_argument('--debug', action='store_true',
                    help='debug mode')
parser.add_argument('--w_state', '--w', default=0.1, type=float,
                    metavar='W', help='weight of state')
...
def main():
    ...
    # args.logname = 'new_' + args.logname + '_' + time_pre + '_' + args.dataset + '_' + args.model + '_' + str(args.num_boxes) + '_' + str(args.num_frames)
    args.logname = 'new_{}_'.format(args.model)
    args.logname += 'bs_{}_'.format(str(args.batch_size))
    args.logname += 'epochs_{}_'.format(str(args.epochs))
    args.logname += 'wstate_{}_'.format(str(args.w_state))
    args.logname += 'lr_{}_'.format(str(args.lr))
    args.logname = args.logname + 'lrsteps_'
    for d in args.lr_steps:
        args.logname += '%d_' % d
    args.logname += time_pre
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, optimizer, epoch, criterion, tb_logger, args.debug)

        # evaluate on validation set
        if (not args.fine_tune) or (epoch + 1) % 10 == 0:
            loss, top1_acc, inst_acc, success_rate, miou = validate(val_loader, model, criterion, epoch=epoch, tb_logger=tb_logger, debug=args.debug)
        else:
            loss = 100
        ...
        if epoch > max(40, args.epochs/2) and is_best_top1_acc:
            save_checkpoint(
                {
                    'epoch': epoch + 1,
...
def train(train_loader, model, optimizer, epoch, criterion, tb_logger=None, debug=False):
    ...
    for i, (global_img_tensors, video_label) in enumerate(train_loader):
        model.zero_grad()
        ...
        if args.model_type in ['woT']:
            ...
            video_label = video_label[:, 1]
        output, state_loss = model(global_img_tensors, video_label)
        ...
        if args.pred_state_action:
            loss = loss + args.w_state * state_loss
        ...
        if debug and i > args.print_freq:
            break
...
def validate(val_loader, model, criterion, epoch=None, tb_logger=None, class_to_idx=None, debug=False):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc_top1 = AverageMeter()
    acc_top5 = AverageMeter()
    ...
    all_pred_inst_list = []
    all_target_inst_list = []
    for i, (global_img_tensors, video_label) in enumerate(val_loader):
        # compute output
        global_img_tensors = global_img_tensors.cuda()
        video_label = video_label.cuda()
        with torch.no_grad():
            if args.dataset == 'crosstask':
                ...
                if args.generation_method == 'autoregression':
                    output, domain_prior_list = model.base.model_get_action(global_img_tensors, video_label, is_inference=True)
                elif args.generation_method == 'non-autoregression':
                    output = model(global_img_tensors, video_label, is_inference=True)
            else:
                output = model(global_img_tensors, video_label, is_inference=True)
        ...
        if debug and i % args.print_freq == 0:
            break
