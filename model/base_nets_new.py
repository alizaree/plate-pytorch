...
class LangDecode(nn.Module):
    def __init__(self, hidden_size=128, max_message_len=None, num_classes=None, sa_type=None, state_size=0):
        ...
        if self.sa_type == 'feature_concat':
            self.gen_comm = nn.Linear(self.state_size, self.num_classes, bias=False) # True?
        else:
    ...
    def get_feat(self, message_input, message_mask_tem=None):
        ...
        split_output = torch.split(message_output, [int(self.state_size/2), int(self.state_size/2), self.state_size], dim=-1)
        state_output_feat, message_output_feat = split_output[1], split_output[2]
